// <auto-generated />
using System;
using LoanRequestInfrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace LoanRequestInfrastructure.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260402132751_Added_DocumentId_docRequirement_Table")]
    partial class Added_DocumentId_docRequirement_Table
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.14")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("LoanRequestDomain.Entities.Applicant", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("BVN")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DateOfBirth")
                        .HasColumnType("date");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Gender")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("KycStatus")
                        .HasColumnType("int");

                    b.Property<DateTime?>("KycVerifiedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("MaritalStatus")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("MiddleName")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("NIN")
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<string>("Nationality")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal>("ProfileCompleteness")
                        .HasColumnType("decimal(5,2)");

                    b.Property<string>("ReferralCode")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("ResidentialAddress")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("ResidentialLGA")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("ResidentialState")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("StateOfOrigin")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("BVN")
                        .IsUnique();

                    b.HasIndex("NIN")
                        .IsUnique()
                        .HasFilter("[NIN] IS NOT NULL");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("Applicant");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.ApplicantEmployment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ApplicantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Department")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("EmployerAddress")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("EmployerName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("EmployerPhone")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime?>("EmploymentStartDate")
                        .HasColumnType("date");

                    b.Property<int>("EmploymentType")
                        .HasColumnType("int");

                    b.Property<bool>("IsCurrentEmployer")
                        .HasColumnType("bit");

                    b.Property<string>("JobTitle")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<decimal>("MonthlyGrossSalary")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("MonthlyNetSalary")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("SalaryAccountBank")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("SalaryAccountNumber")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicantId")
                        .IsUnique();

                    b.ToTable("ApplicantEmployments");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.ApplicantFinancials", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ApplicantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreditBureauRef")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("CreditScore")
                        .HasColumnType("int");

                    b.Property<decimal>("MonthlyObligations")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("OtherMonthlyIncome")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("TotalAssets")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("TotalLiabilities")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("ApplicantId")
                        .IsUnique();

                    b.ToTable("ApplicantFinancials");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.ApplicationDocumentChecklist", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AllowedFileTypes")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DocumentTypeCode")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DocumentTypeName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsMandatory")
                        .HasColumnType("bit");

                    b.Property<Guid>("LoanApplicationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("LoanDocumentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("MaxFileSizeMb")
                        .HasColumnType("int");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("LoanApplicationId")
                        .IsUnique();

                    b.ToTable("ApplicationDocumentChecklists");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.DocumentRequirement", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AllowedFileTypes")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DocumentTypeCode")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DocumentTypeName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsMandatory")
                        .HasColumnType("bit");

                    b.Property<Guid>("LoanProductId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("MaxFileSizeMb")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("LoanProductId");

                    b.ToTable("DocumentRequirements");

                    b.HasData(
                        new
                        {
                            Id = new Guid("11948273-1111-0000-0000-000000000001"),
                            AllowedFileTypes = "pdf,jpg,png",
                            Description = "Provide payslips for the last 3 consecutive months showing employer name and net salary.",
                            DocumentTypeCode = "PAYSLIP",
                            DocumentTypeName = "Last 3 Months Payslip",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-1111-4444-8888-000000000001"),
                            MaxFileSizeMb = 5
                        },
                        new
                        {
                            Id = new Guid("11948273-1111-0000-0000-000000000002"),
                            AllowedFileTypes = "pdf",
                            Description = "Official bank statement showing salary entries for the last 6 months.",
                            DocumentTypeCode = "BANK_STMT",
                            DocumentTypeName = "6 Months Bank Statement",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-1111-4444-8888-000000000001"),
                            MaxFileSizeMb = 10
                        },
                        new
                        {
                            Id = new Guid("11948273-1111-0000-0000-000000000003"),
                            AllowedFileTypes = "pdf,jpg",
                            Description = "Letter from your employer confirming your job status and length of service.",
                            DocumentTypeCode = "EMP_LETTER",
                            DocumentTypeName = "Employment Confirmation Letter",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-1111-4444-8888-000000000001"),
                            MaxFileSizeMb = 2
                        },
                        new
                        {
                            Id = new Guid("11948273-1111-0000-0000-000000000004"),
                            AllowedFileTypes = "pdf,jpg,png",
                            Description = "Valid Passport, Driver's License, or National ID Card.",
                            DocumentTypeCode = "VALID_ID",
                            DocumentTypeName = "Government Issued ID",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-1111-4444-8888-000000000001"),
                            MaxFileSizeMb = 2
                        },
                        new
                        {
                            Id = new Guid("11948273-1111-0000-0000-000000000005"),
                            AllowedFileTypes = "pdf,jpg,png",
                            Description = "Recent utility bill (Electricity, Water, or Waste) not older than 3 months.",
                            DocumentTypeCode = "UTILITY_BILL",
                            DocumentTypeName = "Proof of Address",
                            IsMandatory = false,
                            LoanProductId = new Guid("01948273-1111-4444-8888-000000000001"),
                            MaxFileSizeMb = 2
                        },
                        new
                        {
                            Id = new Guid("11948273-2222-0000-0000-000000000001"),
                            AllowedFileTypes = "jpg,png",
                            Description = "A clear photo of your current staff ID card.",
                            DocumentTypeCode = "COMPANY_ID",
                            DocumentTypeName = "Staff Identity Card",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-2222-4444-8888-000000000002"),
                            MaxFileSizeMb = 2
                        },
                        new
                        {
                            Id = new Guid("11948273-2222-0000-0000-000000000002"),
                            AllowedFileTypes = "pdf,jpg",
                            Description = "Last month's payslip for salary validation.",
                            DocumentTypeCode = "PAYSLIP",
                            DocumentTypeName = "Most Recent Payslip",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-2222-4444-8888-000000000002"),
                            MaxFileSizeMb = 2
                        },
                        new
                        {
                            Id = new Guid("11948273-3333-0000-0000-000000000001"),
                            AllowedFileTypes = "pdf",
                            Description = "Official Certificate of Incorporation from the Corporate Affairs Commission.",
                            DocumentTypeCode = "CAC_CERT",
                            DocumentTypeName = "CAC Certificate",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-3333-4444-8888-000000000003"),
                            MaxFileSizeMb = 5
                        },
                        new
                        {
                            Id = new Guid("11948273-3333-0000-0000-000000000002"),
                            AllowedFileTypes = "pdf",
                            Description = "Corporate bank statement for the last 12 months.",
                            DocumentTypeCode = "BANK_STMT",
                            DocumentTypeName = "12 Months Bank Statement",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-3333-4444-8888-000000000003"),
                            MaxFileSizeMb = 15
                        },
                        new
                        {
                            Id = new Guid("11948273-3333-0000-0000-000000000003"),
                            AllowedFileTypes = "pdf",
                            Description = "Last 2 years audited financial statements signed by a certified accountant.",
                            DocumentTypeCode = "AUDITED_ACCTS",
                            DocumentTypeName = "Audited Accounts",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-3333-4444-8888-000000000003"),
                            MaxFileSizeMb = 10
                        },
                        new
                        {
                            Id = new Guid("11948273-3333-0000-0000-000000000004"),
                            AllowedFileTypes = "pdf,jpg,png",
                            Description = "Valid government ID of at least one director.",
                            DocumentTypeCode = "VALID_ID",
                            DocumentTypeName = "Director's ID",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-3333-4444-8888-000000000003"),
                            MaxFileSizeMb = 2
                        },
                        new
                        {
                            Id = new Guid("11948273-4444-0000-0000-000000000001"),
                            AllowedFileTypes = "pdf,jpg,png",
                            Description = "Passport, Driver's License or National ID for immediate verification.",
                            DocumentTypeCode = "VALID_ID",
                            DocumentTypeName = "Government Issued ID",
                            IsMandatory = true,
                            LoanProductId = new Guid("01948273-4444-4444-8888-000000000004"),
                            MaxFileSizeMb = 2
                        });
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.EligibilityChecks", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ApplicantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("DSRApplied")
                        .HasColumnType("decimal(5,2)");

                    b.Property<decimal>("DisposableIncome")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("EffectiveInterestRate")
                        .HasColumnType("decimal(8,4)");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("nvarchar(45)");

                    b.Property<bool>("IsEligible")
                        .HasColumnType("bit");

                    b.Property<Guid>("LoanProductId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("MaxEligibleAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("MaxMonthlyRepayment")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("MinEligibleAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("MonthlyGrossSalary")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("MonthlyObligations")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("RecommendedAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("RejectionReasons")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("RequestedAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("RequestedTenorMonths")
                        .HasColumnType("int");

                    b.Property<int>("RiskRating")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicantId");

                    b.HasIndex("ExpiresAt");

                    b.HasIndex("LoanProductId");

                    b.ToTable("EligibilityChecks");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.LoanApplication", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ApplicantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ApplicationNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<decimal?>("ApprovedAmount")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<bool>("BlacklistChecked")
                        .HasColumnType("bit");

                    b.Property<string>("BlacklistResult")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int?>("BureauScoreUsed")
                        .HasColumnType("int");

                    b.Property<DateTime?>("ConsentAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("ConsentGiven")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DecisionAt")
                        .HasColumnType("datetime2");

                    b.Property<Guid?>("DecisionBy")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("EligibilityCheckId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("InterestRate")
                        .HasPrecision(8, 4)
                        .HasColumnType("decimal(8,4)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<Guid>("LoanProductId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("LoanPurpose")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<decimal>("MonthlyRepayment")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("PreScreeningResult")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("RequestedAmount")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<string>("StatusReason")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTime?>("SubmittedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("TenorMonths")
                        .HasColumnType("int");

                    b.Property<decimal>("TotalRepayable")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("ApplicantId");

                    b.HasIndex("ApplicationNumber")
                        .IsUnique();

                    b.HasIndex("EligibilityCheckId")
                        .IsUnique();

                    b.HasIndex("LoanProductId");

                    b.ToTable("LoanApplications");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.LoanProducts", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("EligibilityCriteria")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("InterestRatePercent")
                        .HasColumnType("decimal(8,4)");

                    b.Property<int>("InterestRateType")
                        .HasColumnType("int");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int>("LoanType")
                        .HasColumnType("int");

                    b.Property<decimal>("MaxAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("MaxDSRPercent")
                        .HasColumnType("decimal(5,2)");

                    b.Property<decimal>("MaxLTIMultiplier")
                        .HasColumnType("decimal(5,2)");

                    b.Property<int>("MaxTenorMonths")
                        .HasColumnType("int");

                    b.Property<decimal>("MinAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("MinTenorMonths")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("ProductCode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("RequiredDocumentTypes")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("LoanProducts");

                    b.HasData(
                        new
                        {
                            Id = new Guid("01948273-1111-4444-8888-000000000001"),
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            InterestRatePercent = 18.00m,
                            InterestRateType = 0,
                            IsActive = true,
                            LoanType = 0,
                            MaxAmount = 50000000m,
                            MaxDSRPercent = 33.00m,
                            MaxLTIMultiplier = 60.00m,
                            MaxTenorMonths = 60,
                            MinAmount = 50000m,
                            MinTenorMonths = 6,
                            Name = "Personal Loan",
                            ProductCode = "PLN001",
                            RequiredDocumentTypes = "[\"GovID\", \"UtilityBill\", \"3MonthsPayslip\"]"
                        },
                        new
                        {
                            Id = new Guid("01948273-2222-4444-8888-000000000002"),
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            InterestRatePercent = 5.00m,
                            InterestRateType = 2,
                            IsActive = true,
                            LoanType = 1,
                            MaxAmount = 1000000m,
                            MaxDSRPercent = 40.00m,
                            MaxLTIMultiplier = 3.00m,
                            MaxTenorMonths = 12,
                            MinAmount = 10000m,
                            MinTenorMonths = 1,
                            Name = "Salary Advance",
                            ProductCode = "SADV001",
                            RequiredDocumentTypes = "[\"CompanyID\", \"1MonthPayslip\"]"
                        },
                        new
                        {
                            Id = new Guid("01948273-3333-4444-8888-000000000003"),
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            InterestRatePercent = 22.00m,
                            InterestRateType = 1,
                            IsActive = true,
                            LoanType = 2,
                            MaxAmount = 100000000m,
                            MaxDSRPercent = 35.00m,
                            MaxLTIMultiplier = 0.00m,
                            MaxTenorMonths = 84,
                            MinAmount = 500000m,
                            MinTenorMonths = 12,
                            Name = "Business Loan",
                            ProductCode = "BIZ001",
                            RequiredDocumentTypes = "[\"CAC_Docs\", \"6MonthsBankStatement\"]"
                        },
                        new
                        {
                            Id = new Guid("01948273-4444-4444-8888-000000000004"),
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            InterestRatePercent = 10.00m,
                            InterestRateType = 2,
                            IsActive = true,
                            LoanType = 5,
                            MaxAmount = 100000m,
                            MaxDSRPercent = 50.00m,
                            MaxLTIMultiplier = 1.00m,
                            MaxTenorMonths = 6,
                            MinAmount = 5000m,
                            MinTenorMonths = 1,
                            Name = "Emergency Loan",
                            ProductCode = "EMG001",
                            RequiredDocumentTypes = "[\"GovID\"]"
                        });
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.RefreshToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("DeviceInfo")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("RevokedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Token")
                        .IsRequired()
                        .HasMaxLength(512)
                        .HasColumnType("nvarchar(512)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("Token")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.ToTable("RefreshTokens");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.Applicant", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", "User")
                        .WithOne()
                        .HasForeignKey("LoanRequestDomain.Entities.Applicant", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.ApplicantEmployment", b =>
                {
                    b.HasOne("LoanRequestDomain.Entities.Applicant", "Applicant")
                        .WithOne("Employment")
                        .HasForeignKey("LoanRequestDomain.Entities.ApplicantEmployment", "ApplicantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Applicant");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.ApplicantFinancials", b =>
                {
                    b.HasOne("LoanRequestDomain.Entities.Applicant", "Applicant")
                        .WithOne("Financials")
                        .HasForeignKey("LoanRequestDomain.Entities.ApplicantFinancials", "ApplicantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Applicant");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.ApplicationDocumentChecklist", b =>
                {
                    b.HasOne("LoanRequestDomain.Entities.LoanApplication", "LoanApplication")
                        .WithOne("DocumentChecklist")
                        .HasForeignKey("LoanRequestDomain.Entities.ApplicationDocumentChecklist", "LoanApplicationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("LoanApplication");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.DocumentRequirement", b =>
                {
                    b.HasOne("LoanRequestDomain.Entities.LoanProducts", "LoanProduct")
                        .WithMany("DocumentRequirements")
                        .HasForeignKey("LoanProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("LoanProduct");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.EligibilityChecks", b =>
                {
                    b.HasOne("LoanRequestDomain.Entities.Applicant", "Applicant")
                        .WithMany("EligibilityChecks")
                        .HasForeignKey("ApplicantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LoanRequestDomain.Entities.LoanProducts", "LoanProduct")
                        .WithMany()
                        .HasForeignKey("LoanProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Applicant");

                    b.Navigation("LoanProduct");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.LoanApplication", b =>
                {
                    b.HasOne("LoanRequestDomain.Entities.Applicant", "Applicant")
                        .WithMany("LoanApplications")
                        .HasForeignKey("ApplicantId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("LoanRequestDomain.Entities.EligibilityChecks", "EligibilityCheck")
                        .WithMany()
                        .HasForeignKey("EligibilityCheckId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LoanRequestDomain.Entities.LoanProducts", "LoanProduct")
                        .WithMany()
                        .HasForeignKey("LoanProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Applicant");

                    b.Navigation("EligibilityCheck");

                    b.Navigation("LoanProduct");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.RefreshToken", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.Applicant", b =>
                {
                    b.Navigation("EligibilityChecks");

                    b.Navigation("Employment");

                    b.Navigation("Financials");

                    b.Navigation("LoanApplications");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.LoanApplication", b =>
                {
                    b.Navigation("DocumentChecklist");
                });

            modelBuilder.Entity("LoanRequestDomain.Entities.LoanProducts", b =>
                {
                    b.Navigation("DocumentRequirements");
                });
#pragma warning restore 612, 618
        }
    }
}
